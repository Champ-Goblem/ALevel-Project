...
Public Class DatabaseTools
    ...
    Public Sub createDatabase(ByVal filePath As String, ByVal databaseFile As databaseEnum)
        ...
        If databaseFile = databaseEnum.ClientDatabase Then
            ...
            sendSQLNonQuery(SQL)
            SQL = "CREATE TABLE TBL_MESSAGES ([MessageID] COUNTER NOT NULL UNIQUE PRIMARY KEY, [MessageData] LONGTEXT, [ReceivedDate] LONGTEXT, [Sender_UserID] LONGTEXT, CONSTRAINT FK_SenderUsername2 FOREIGN KEY ([Sender_UserID]) REFERENCES TBL_USERS([Username]));"
            sendSQLNonQuery(SQL)
        Else
            'SQL = "CREATE DATABASE " & Path.GetFileNameWithoutExtension(filePath) & ";"
            'sendSQLNonQuery(SQL)
            SQL = "CREATE TABLE TBL_USER ([Username] TEXT NOT NULL PRIMARY KEY, [Password_Hash] LONGTEXT NOT NULL, [Is_Administrator] YESNO, [Last_Known_PublicKey] LONGTEXT);"
            sendSQLNonQuery(SQL)
            SQL = "INSERT INTO TBL_USER ([Username], [Password_Hash], [Is_Administrator]) VALUES ('Administrator', 'CvsAE42OczSOwf5B/T06j8vZAVayY7+leRug4JX0LPw=', TRUE);"
            sendSQLNonQuery(SQL)
            SQL = "CREATE TABLE TBL_UNSENT_FILES ([FileID] COUNTER NOT NULL UNIQUE PRIMARY KEY, [Filepath] LONGTEXT, [Sender_UserID] TEXT, [Recipient_UserID] TEXT, CONSTRAINT FK_SenderUsername FOREIGN KEY ([Sender_UserID]) REFERENCES TBL_USER([Username]), CONSTRAINT FK_RecipientUsername FOREIGN KEY ([Recipient_UserId]) REFERENCES TBL_USER([Username]));"
            sendSQLNonQuery(SQL)
            SQL = "CREATE TABLE TBL_UNSENT_MESSAGES ([MessageID] COUNTER NOT NULL UNIQUE PRIMARY KEY, [MessageData] LONGTEXT, [Sender_UserID] TEXT, [Recipient_UserID] TEXT, CONSTRAINT FK_SenderUsername2 FOREIGN KEY ([Sender_UserID]) REFERENCES TBL_USER([Username]), CONSTRAINT FK_RecipientUsername2 FOREIGN KEY ([Recipient_UserId]) REFERENCES TBL_USER([Username]));"
            sendSQLNonQuery(SQL)
        End If
    End Sub
